...
class RunningAppsObserver {
  ...
  init(workspace: NSWorkspace = .shared) {
    ...
    let windowChangeObservers = Mutex([pid_t: WindowChangeObserver?]())
    self.rawObserver = self.workspace.observe(\.runningApplications) {
      workspace,
      _ in
      windowChangeObservers.withLock { windowChangeObservers in
        let oldKeys = Set(windowChangeObservers.keys)
        let newKeys = getWindowChangePIDs(for: workspace.runningApplications)
